...
func TestSetAndGet(t *testing.T) {
	...
	_, err := stringService.Set("", "")
	if err == nil || err.Error() != common.ErrKeyEmpty {
		t.Errorf("Test Set string failed, got: %s, want: %s", err.Error(), common.ErrKeyEmpty)
	}

	value1, err := stringService.Get("key1")
	if value1 != "value1" || err != nil {
		t.Errorf("Test Get string failed, got: %s, want: %s", value1, "value1")
	}
	value2, err := stringService.Get("key2")
	if value2 != "value3" || err != nil {
		t.Errorf("Test Get string failed, got: %s, want: %s", value2, "value3")
	}
	value3, err := stringService.Get("key3")
	if value3 != "value4 and 5" || err != nil {
		t.Errorf("Test Get string failed, got: %s, want: %s", value3, "value4 and 5")
	}
	_, err = stringService.Get("key4")
	if err == nil || err.Error() != common.ErrKeyNotFound {
		t.Errorf("Test Get string failed, got: %s, want: %s", err.Error(), common.ErrKeyNotFound)
	}
	_, err = stringService.Get("")
	if err == nil || err.Error() != common.ErrKeyEmpty {
		t.Errorf("Test Get string failed, got: %s, want: %s", err.Error(), common.ErrKeyEmpty)
	}
}
